...
**Hidden vault (deniability).** `VaultService` manages one **primary** vault plus at most one optional **hidden** vault, stored at `VaultPaths.forHidden()` (`<appSupport>/.sdk_sys/`, a *sibling* of the primary root so primary backup/`destroy()` never touch it). `unlock(password)` tries the primary descriptor, then the hidden one — typing the hidden password on the normal lock screen opens the hidden vault, with `_active` (and `blobStore`/`db`/keys) following it. It is created/replaced via `createHiddenVault` (offered at onboarding and in Settings) and removed via `destroyHidden`. `RegisterFailedUnlockUseCase` calls `destroyHidden` on every panic threshold hit (3 wrong attempts), unconditionally — the hidden vault self-destructs under duress regardless of the normal vault's lockout/wipe setting. This is *behavioural* deniability (hidden from the app UI/registry and from primary backups), **not** forensic-grade — the `.sdk_sys` directory is still visible on disk.
